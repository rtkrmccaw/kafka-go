...
func (r *reader) read(ctx context.Context, offset int64, conn *Conn) (int64, error) {
	...
	for {
		...
		if err = r.sendMessage(ctx, msg, highWaterMark); err != nil {
			r.withErrorLogger(func(log Logger) {
				log.Printf("failed to send message at offset %d for topic %s and partition %d and highWaterMark %d: %s", toHumanOffset(offset), r.topic, r.partition, highWaterMark, err)
			})
			batch.Close()
			break
		}
